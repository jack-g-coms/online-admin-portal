...
import { updateWarning } from '../modules/RobloxModerations';

function ApplyModifyRobloxWarningPopup({setState, editedWarning, changes}) {
        ...
        updateWarning(editedWarning.rbxID, editedWarning.warnID, editedWarning.moderator, editedWarning.evidence, editedWarning.reason)
            .then(response => {
                ...
                if (response.message == 'Not Found') {
                    Swal.fire({title: 'Error', icon: 'error', text: `No Warning under the ID ${editedWarning.warnID} was found. This warning could've already been deleted.`, confirmButtonText: 'Ok'});
                } else if (response.message == 'Success') {
                    Swal.fire({title: 'Success', text: 'Warning has been successfully updated per your specifications.', icon: 'success', confirmButtonText: 'Ok'})
                        .then(() => {
...
export default ApplyModifyRobloxWarningPopup;