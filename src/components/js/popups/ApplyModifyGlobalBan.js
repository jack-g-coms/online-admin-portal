...
function ApplyModifyGlobalBanPopup({setState, setActionState, setEditorState, editedBan, changed}) {
    const [popupState, setPopupState] = useState('available');

    const applyChanges = () => {
        DiscordModeration.updateGlobalBan(editedBan.discordID, editedBan.moderator, editedBan.reason)
            .then(response => {
                if (response.message == 'Not Found') {
                    Swal.fire({title: 'Error', icon: 'error', text: `No Global Ban under the ID ${editedBan.discordID} was found. This global ban could've already been deleted.`, confirmButtonText: 'Ok'});
                } else if (response.message == 'Success') {
                    Swal.fire({title: 'Success', text: 'Global Ban has been successfully updated per your specifications.', icon: 'success', confirmButtonText: 'Ok'})
                        .then(() => {
                            window.location.reload();
                        });
                } else {
                    Swal.fire({title: 'Error', icon: 'error', text: `There was a problem while trying to update this global ban.`, confirmButtonText: 'Ok'});
                }
                setState('closed');
            }).catch(console.log);
    };
...
export default ApplyModifyGlobalBanPopup;