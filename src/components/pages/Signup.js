    ...
    return (
        <>
            <div className='signup-form-container'>
                ...
                <div className='signup-container-fields'>
                    <div className='signup-container-field'>
                        <span><i style={{'marginRight': '3px'}} class='fa-solid fa-envelope'/> Enter Email</span>
                        <TextBox style={{'width': '100%', 'text-align': 'center'}} placeholder='Input Email' type='email' setState={setEmail}>{email}</TextBox>
                    </div>

                    <div className='signup-container-field'>
                        <span><i style={{'marginRight': '3px'}} class='fa-solid fa-user'/> Enter ROBLOX Username</span>
                        <TextBox style={{'width': '100%', 'text-align': 'center'}} placeholder='Input Username' setState={setUsername}>{username}</TextBox>
                    </div>

                    <div className='signup-container-field'>
                        <span><i style={{'marginRight': '3px'}} class='fa-solid fa-lock'/> Enter Password</span>
                        <TextBox style={{'width': '100%', 'text-align': 'center'}} placeholder='Input Password' type='password' setState={setPassword}>{password}</TextBox>
                    </div>

                    <Button animation='raise' style={{'width': '100%'}} onClick={(e) => {setState('loading'); triggerAccountSignup();}} scheme='btn-confirm'>{state == 'available' && <>Create Account <i class='fa-solid fa-arrow-right'/></> || state == 'loading' && <i className='fa-solid fa-spinner loader'/>}</Button>
